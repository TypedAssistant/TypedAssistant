...
clone_and_copy_template() {
    git clone --depth 1 "https://github.com/TypedAssistant/TypedAssistant.git" /ta-repo
    cp -ra /ta-repo/packages/template/. /config/TypedAssistant
    rm -rf /ta-repo
}

setup_and_push_to_backup_repo() {
    cd /config/TypedAssistant
    git init
    git add .
...
if [ ! -d "/config/TypedAssistant" ] || [ -z "$(ls -A /config/TypedAssistant)" ]; then
    echo "TypedAssistant directory does not exist"
    clone_and_copy_template
fi
...
if [[ -n "$GITHUB_TOKEN_FROM_CONFIG" && -n "$GITHUB_USERNAME_FROM_CONFIG" && -n "$GITHUB_REPO_FROM_CONFIG" && -n "$GITHUB_BRANCH_FROM_CONFIG" ]]; then
    ...
    if [ ! -d "/config/TypedAssistantBackup" ] || [ -z "$(ls -A /config/TypedAssistantBackup)" ]; then
        ...
        rm -rf /config/TypedAssistant
        cp -ra /config/TypedAssistantBackup/. /config/TypedAssistant
        cd /config/TypedAssistant
...
## SETUP DEPENDENCIES
bun preinstall.tsx /config/TypedAssistant/package.json
cp install-deps.tsx /config/TypedAssistant/install-deps.tsx
cd /config/TypedAssistant
bun install-deps.tsx
rm install-deps.tsx
